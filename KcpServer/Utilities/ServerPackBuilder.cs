...
{
    /// <summary>
    /// 基本协议,sysid(4bytes),sessionid(4bytes),length(4bytes)
    /// </summary>
    /// <remarks>
    /// 不可以超过1000字节,否则用其他Builder替换它
    /// </remarks>
    public class ServerPackBuilder
    {
        ...
        public ServerPackBuilder(byte a, byte b, byte c, byte d, int sid)
        {
            SysIdBuf[0] = a;
            SysIdBuf[1] = b;
            SysIdBuf[2] = c;
        ...
        public ServerPackBuilder(byte[] arr, int sid)
        {
